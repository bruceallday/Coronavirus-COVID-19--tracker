    ...
    return (
      hoveredFeature && (
        <div className={classes.tooltip} style={{ left: x, top: y }}>
          ...
          <div>Active cases:
            <span
              className={hoveredFeature.properties.active == 0 ? (
                textClass.blueText
              ) : (
                  textClass.yellowText
                )}>
              {formatNumber(hoveredFeature.properties.active)}
            </span>
          </div>

          <div>Total cases:
            <span
              className={textClass.orangeText}>
          ...
          <div>Deaths:
            <span
              className={textClass.redtext}>
              {formatNumber(hoveredFeature.properties.deaths)}
            </span>
          </div>
        </div>
      )
