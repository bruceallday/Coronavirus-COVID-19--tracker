  ...
  return (
    <Paper elevation={3} className={classes.root}>
      ...
      <Typography style={{ paddingLeft: 5 }} variant="body1" >
        {`Active Cases:`}
        <span className={active === 0 ? (textClasses.blueText) : (textClasses.yellowText)} >
          {`${formatNumber(active)}`}
        </span>
      </Typography>

      <Typography style={{ paddingLeft: 5 }} variant="body1" >
        {`Total Cases: ${formatNumber(cases)} | Today: `}
        <span className={todayCases == 0 ? (
          textClasses.greenText
        ) : (
            textClasses.orangeText
          )
      ...
      <Typography style={{ paddingLeft: 5 }} variant="body1" >
        {`Recovered: `}
        <span className={textClasses.greenText}>
          {`${formatNumber(recovered) === 0 ? (
            'N/A'
          ) : (
      ...
      <Typography style={{ paddingLeft: 5 }} variant="body1" >
        {`Deaths: ${formatNumber(deaths)} | Today: `}
        <span className={todayDeaths == 0 ? (
          textClasses.greenText
        ) : (
            textClasses.redtext
